...
<head>
  ...
  <!-- reCAPTCHA (kept as you had, only once) -->
...
<body style="background-color:#FFF9F3;">
  ...
  <div class="header pt-5">
    <h2 class="mt-lg-5">Einstar Rockit</h2>
  </div>

  <?php
  // Fetch product (hard-coded id=7 as in your original)
  $select_cart_product = mysqli_query($con, "SELECT * FROM `einscan hx` WHERE id = 9");
  $product_title = '';
  $total_quantity = 0;
  $subtotal = 0.0; // INR
  ...
  if ($select_cart_product && mysqli_num_rows($select_cart_product) > 0) {
    while ($row = mysqli_fetch_assoc($select_cart_product)) {
        ...
        // if qty=... is in URL, use it; otherwise fall back to DB quantity
...
// Apply coupon discount AFTER we know total_quantity
if ($applied_coupon && preg_match('/^DIS(\d{2,7})$/', $applied_coupon, $m)) {
    $flatPerUnit = (int)$m[1];                  // e.g. DIS50000 → 50000 per unit
    $subtotal   -= $flatPerUnit * $total_quantity; // multiply by quantity
  ...
  // Currency conversion INR -> USD using exchangerate-api
  function getConversionRateUSDPerINR() {
    // NOTE: consider moving key to server env
    ...
    if ($data && isset($data['conversion_rates']['USD'])) {
      return (float)$data['conversion_rates']['USD']; // USD per 1 INR
    }
    return false;
  }
  ...
  <div class="container pt-5 pb-5">
    <!-- Single form wrapping both columns (correct) -->
    <form id="paymentForm" action="create_payment.php" method="post">
      <div class="row">
        <div class="col-lg-6">
          <div class="payment-form">
            <div class="form-1">
              ...
              <!-- Hidden product fields for PayPal (and optionally server) -->
        ...
        <div class="col-lg-6">
          ...
          <!-- PayPal submit (posts the form to create_payment.php) -->
          <button class="pay-btn" type="submit">Pay with PayPal</button>

          <!-- Razorpay button (handled via JS) -->
  ...
  <script>
    ...
    $('body').on('click', '.buy_now', function (e) {
      ...
      var comp_name   = $('#comp_name').val().trim(); // FIXED id
      ...
      var totalAmount      = $(this).attr("data-amount"); // INR total
      ...
      var options = {
        ...
        "name": "Einstar Rockit",
        "description": "Payment",
        "image": "https://www.precise3dm.com/assets/images/einstar-rockit/pro1.png",
        "handler": function (response) {
          $.ajax({
            ...
            success: function (msg) {
              // TODO: replace with your own success page
  ...
  <script>
    ...
    document.getElementById("sameAddress").addEventListener("change", function () {
      ...
      if (this.checked) {
        ...
        delivery.removeAttribute("readonly"); // FIXED: remove from delivery
        delivery.value = "";
      }
    });
  </script>
