...
<body style="background-color:#FFF9F3;">
  ...
  <div class="header pt-5">
    <h2 class="mt-lg-5">Einstar Vega</h2>
  </div>

  <?php
  // Fetch product (hard-coded id=7 as in your original)
  $select_cart_product = mysqli_query($con, "SELECT * FROM `einscan hx` WHERE id = 8");
  $product_title = '';
  $total_quantity = 0;
  $subtotal = 0.0; // INR
  ...
  if ($select_cart_product && mysqli_num_rows($select_cart_product) > 0) {
    while ($row = mysqli_fetch_assoc($select_cart_product)) {
      $title = isset($row['title']) ? $row['title'] : 'Product';
      $qty   = isset($row['quantity']) && (int)$row['quantity'] > 0 ? (int)$row['quantity'] : 1; // default to 1
      $price = isset($row['price']) ? (float)$row['price'] : 0.0; // INR per unit

      $product_title = $title; // last/only row
      $total_quantity += $qty;
      $subtotal += ($price * $qty);
    }
  }
  ...
  <script>
    ...
    $('body').on('click', '.buy_now', function (e) {
      ...
      var options = {
        ...
        "name": "Einstar Vega",
        "description": "Payment",
        "image": "https://www.precise3dm.com/assets/images/einstar-vega-product/pro1.png",
